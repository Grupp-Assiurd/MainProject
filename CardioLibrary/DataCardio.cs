...
namespace CardioLibrary
{
    public class DataCardio
    {
        ...
        public static double CalorieBruciate(bool isUomo, int frequenza, float peso, int eta, double durata)
        {
            double calcoloTempo = durata * 4.184;
            double calorie;
            if (isUomo)
            {
                calorie = ((eta * 0.2017) + (peso * 0.199) + (frequenza * 0.6309) - 55.0969) * calcoloTempo;
            }
            else
            {
                calorie = ((eta * 0.074) + (peso * 0.126) + (frequenza * 0.4472) - 29.4022) * calcoloTempo;
            }

            return Math.Round(calorie, 2);
